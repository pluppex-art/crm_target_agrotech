...
const columns: ColumnDef<Transaction>[] = [
  ...
  { accessorKey: "date", header: "Data", cell: ({ row }) => formatDate(row.original.date) },
  {
    accessorKey: "amount",
    header: "Valor",
    cell: ({ row }) => <span className="text-red-600 font-semibold">{formatBRL(row.original.amount)}</span>,
  },
  {
    accessorKey: "status",
    header: "Status",
...
export default function ExpensesPage() {
  const { transactions, isLoading, createTransaction } = useTransactions("expense");
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);

  return (
    <div>
      <PageHeader title="Despesas" description="Saídas financeiras">
      ...
      {isLoading ? (
        ...
        <DataTable columns={columns} data={transactions} searchKey="description" searchPlaceholder="Buscar despesa..." />
      )}
